...
def judge_caption_board_closeup(bboxes, img_w, img_h, threshold_closeup=0.3, threshold_kanrichi=0.8, image_path=None):
    ...
    img_w, img_h: 画像サイズ
    threshold_closeup: 接写判定の閾値（例: 0.3）
    threshold_kanrichi: 管理値（最アップ）判定の閾値（例: 0.8）
    戻り値: (is_closeup, ratio)
      - ratio >= threshold_kanrichi: (None, ratio)  # 管理値（最アップ）
      - ratio >= threshold_closeup: (True, ratio)  # 接写
      - ratio < threshold_closeup: (False, ratio)  # 全景
    """
    # --- 温度計・thermometerがroleに含まれていれば即リターン ---
    for b in bboxes:
        role = b.get("role")
        if role and ("温度計" in role or "thermometer" in role):
            print("[判定ログ] 温度計/thermometerロール検出: 判定スキップ")
            return None, None
    # 画像サイズが未取得ならimage_pathから取得
