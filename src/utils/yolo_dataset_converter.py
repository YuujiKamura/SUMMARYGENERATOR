...
# --- クラス名リスト生成 ---
def load_class_names_filtered(role_mapping_path: str, preset_roles_path: str, used_class_names: set) -> List[str]:
...
def convert_to_yolo_dataset(
    ...
    class_names = load_class_names_filtered(role_mapping_path, preset_roles_path, used_class_names)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="YOLOデータセット変換ツール")
    parser.add_argument("--in_json", required=True, help="画像リストJSON")
    parser.add_argument("--out_dir", default=None, help="出力先ディレクトリ（省略時はdatasets/日付/）")
    parser.add_argument("--val_ratio", type=float, default=0.2, help="バリデーション比率")
    parser.add_argument("--force_flush", action="store_true", help="出力先を強制上書き")
    parser.add_argument("--classify_mode", action="store_true", help="分類タスク用ディレクトリ構造を生成")
    args = parser.parse_args()
    out_dir = Path(args.out_dir) if args.out_dir else get_default_output_dir()
    pm = PathManager()
    role_mapping_path = pm.role_mapping
    preset_roles_path = pm.preset_roles
    result = convert_to_yolo_dataset(
        args.in_json, out_dir, str(role_mapping_path), str(preset_roles_path), args.val_ratio, args.force_flush, args.classify_mode
    )
    print("変換完了:", result)
